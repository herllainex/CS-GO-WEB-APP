...
LinearSearch.prototype.initControls = function() {
	...
	this.initMaxSizeButton = addInputToAlgorithmBar("button", "Initialize Max Size");
	this.initMaxSizeButton.onclick = this.initMaxSizeCallBack.bind(this);
	addLabelToAlgorithmBar("Values");
	this.insertField_value = addInputToAlgorithmBar("text", "");
	this.initArrayButton = addInputToAlgorithmBar("button", "Initialize Array");
	this.initArrayButton.onclick = this.initArrayCallBack.bind(this);
	addLabelToAlgorithmBar("Search Number");
	this.insertField_number = addInputToAlgorithmBar("text", "");
	this.initNumberButton = addInputToAlgorithmBar("button", "Initialize Search Number");
	this.initNumberButton.onclick = this.initNumberCallBack.bind(this);
	this.searchNumberButton = addInputToAlgorithmBar("button", "Search");
...
LinearSearch.prototype.initArray = function( arrayNodeValue ) {
	...
	if( i < this.maxSize ){		
		...
		// 创建状态框数组越界
		{
		    this.cmd("SetState",0,"Out of bounds" );
			this.cmd("Step");
		}
		}
	return this.commands ;
...
LinearSearch.prototype.searchNumber = function ( ){	
  	for(var i = 0 ; i < this.maxSize ; i ++ ){		
		...
		if( this.numNode.value == this.arrayList[i].value){
			// 创建状态框找到该元素
			{
		  		this.cmd("SetState",0,"Element Found");
				this.cmd("Step");
			}
			// 将匹配数字和数组元素背景色变绿
			{
		...
		if( i != -1 ){
			// 创建状态框查找失败
			{
		   		this.cmd("SetState",0,"Element not Found in Array" );
				this.cmd("Step");
			}
		}
		return this.commands ;
